...
SHEET = "Sheet" #Sheet
TICKER_COLUMN = 1
RESULTS_COLUMN = 2#the start of the results columns
...
def getTickers(cwd):
    ...
    workbook = load_workbook(filename = cwd+"\\"+FILE, data_only=True)
...
def printExcel(price):
    ...
    for i in range(len(price)):
        for j in range(len(price[i])):
            ...
            sheet.cell(row=j+2, column=RESULTS_COLUMN+i).value = price[i][j]
            #print("ive added "+str(price[i])+" to "+str(sheet.cell(row=i+2, column=3)))
        

    workbook.save(filename=cwd+"/"+FILE)
